...
class kNN:
    ...
    def run(self):
        ...
        self.dlg.ui.comboBoxMetric.clear()
        self.dlg.ui.comboBoxWeights.clear()
        ...
        #Set criterions Split comboBoxMetric
        self.dlg.ui.comboBoxMetric.addItems(['All','Manhattan (p=1)', 'Euclidean (p=2)','Minkowski (p=3)'])
        #Set criterions Split comboBoxMetric
        self.dlg.ui.comboBoxWeights.addItems(['uniform', 'distance'])
        ...
        self.dlg.ui.spinBoxStartK.valueChanged.connect(self.value_changed_start_k)
    ...
    def set_classification_path (self):
        ...
        if out_file[-1].split('*')[-1].endswith('.shp)'):
            ...
            pass
    def value_changed_start_k(self):
                #Set minimum valspinBoxEnd
        self.dlg.ui.spinBoxEndK.setMinimum(self.dlg.ui.spinBoxStartK.value())
        #Set maximum valspinBoxStep
        self.dlg.ui.spinBoxStepK.setMaximum(self.dlg.ui.spinBoxEndK.value())
        return 0
    def cancel_GUI(self):
    ...
    def run_classification(self):
        ...
        if self.dlg.ui.comboBoxTrain.currentText() == 'None':
            ...
            #Values n_eneigbors
            start_k,end_k,step_k = self.dlg.ui.spinBoxStartK.value(),\
            self.dlg.ui.spinBoxEndK.value(),self.dlg.ui.spinBoxStepK.value()
            #Get Metric
            metric_distance = self.dlg.ui.comboBoxMetric.currentText()
            #Get Weigth
            weigth_features = self.dlg.ui.comboBoxWeights.currentText()
            ...
            #Read file Shell
            path_shell = plugins_path+'python/plugins/knn/run_shell.sh'
            shell=open(path_shell,'w') 
            #Get path python file (k-NN Classifier)
            knn_py_path =plugins_path+'python/plugins/knn/kNNC.py'
            print ('rfc: ',knn_py_path)
            ...
            #arg[12]=output_classification
            
            shell.write('python3 '+knn_py_path+ ' '+train_path+ ' '+dataset_path+ ' '+\
                        validation_path+ ' '+str(start_k)+' '+str(end_k)+' '+str(step_k)+' '+\
                        field_class_train+' '+field_class_val+ ' '+metric_distance+' '+weigth_features+' '+\
